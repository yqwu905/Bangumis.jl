...
function Base.showerror(io::IO, e::DatabaseError)
    return print(io, "Database $(e.db.file): $(e.msg)")
end
...
"""
function generate_create_tbl_statement(
    tbl_name::AbstractString,
    tbl_cols::DataFrame,
)::AbstractString
    create_statement = "CREATE TABLE \"$tbl_name\"(\n"
    pk = ""
    for i ∈ 1:size(tbl_cols, 1)
...
function prepare_db(
    ...
    if ("SUBJECTS" ∉ tbls_name)
        ...
        SQLite.execute(
            db,
            generate_create_tbl_statement("SUBJECTS", SUBJECTS_TBL_DF),
        )
    else
    ...
    if ("EPISODES" ∉ tbls_name)
        ...
        SQLite.execute(
            db,
            generate_create_tbl_statement("EPISODES", EPISODES_TBL_DF),
        )
    else
